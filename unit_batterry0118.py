...
for i in range(4):
    bottom_x_index = np.append(
        bottom_x_index, np.arange(i * 388, i * 388 + 20 * 17, 17)
    )
...
total_heat_train_Nu = total_heat_train[
    idx_Nu, :
].float()  # Training Points  of y at (IC+BC)
...
train_data = DictDataset(
    {"t": T_train_Nf, "y": Y_train_Nf, "x": X_train_Nf, "z": Z_train_Nf}, name="train"
)
# test dataset
test_data = DictDataset(
    {"t": T_test, "y": Y_test, "x": X_test, "z": Z_test, "temperature": temp_test},
    name="test",
)
...
# PDE IC and BC loss
ell_u = (
    scaling * (temperature_hat[-Nu:] == temp_train_Nu) ^ 2
